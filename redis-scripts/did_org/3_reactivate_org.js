...
async function publishEvent(targetWalletAddress, ownerPrivateKey, orgDIDIndex, orgDID, nonce = "0") {
    const provider = new JsonRpcProvider(RPC_URL);
    const ownerWallet = new Wallet(ownerPrivateKey);
    const currentBlockNum = await provider.getBlockNumber();
    const currentBlock = (currentBlockNum + 100).toString(); 

    const txType = 2; // REACTIVATE
    ...
    const event = {
        id: `evt_reactivate_org_${Date.now()}`,
        type: "REACTIVATE_ORG",
        payload: {
    ...
    try {
        ...
        console.log(`✅ Success! REACTIVATE_ORG Redis Message ID: ${messageId}`);
...
async function run() {
    ...
    if (args.length < 4) {
        console.error("Usage: node 3_reactivate_org.js <SmartWallet> <OwnerKey> <OrgDIDIndex> <OrgDID> [nonce]");
        process.exit(1);
    }
