...
function encodeRemoveMember(orgDIDIndex, userDID) {
    const abiCoder = AbiCoder.defaultAbiCoder();
    const userDIDHash = keccak256(solidityPacked(["string"], [userDID]));
    const encodedData = abiCoder.encode(["bytes32"], [userDIDHash]);
...
async function publishEvent(targetWalletAddress, ownerPrivateKey, orgDIDIndex, userDID, nonce = "0") {
    const provider = new JsonRpcProvider(RPC_URL);
    const ownerWallet = new Wallet(ownerPrivateKey);
    const currentBlockNum = await provider.getBlockNumber();
    const currentBlock = (currentBlockNum + 100).toString(); 

    const { callData, innerData } = encodeRemoveMember(orgDIDIndex, userDID);
    ...
    const event = {
        id: `evt_remove_member_${Date.now()}`,
        type: "REMOVE_MEMBER",
        payload: {
    ...
    try {
        ...
        console.log(`✅ Success! REMOVE_MEMBER Redis Message ID: ${messageId}`);
...
async function run() {
    ...
    if (args.length < 4) {
        console.error("Usage: node 6_remove_member.js <SmartWallet> <OwnerKey> <OrgDIDIndex> <UserDID> [nonce]");
        process.exit(1);
    }
    await publishEvent(args[0], args[1], args[2], args[3], args[4] || "0");
    redis.quit();
}
