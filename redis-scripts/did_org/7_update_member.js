...
function encodeUpdateMember(orgDIDIndex, userDID, role) {
...
async function publishEvent(targetWalletAddress, ownerPrivateKey, orgDIDIndex, userDID, role, nonce = "0") {
    ...
    const { callData, innerData } = encodeUpdateMember(orgDIDIndex, userDID, role);
    ...
    const event = {
        id: `evt_update_member_${Date.now()}`,
        type: "UPDATE_MEMBER",
        payload: {
    ...
    try {
        ...
        console.log(`✅ Success! UPDATE_MEMBER Redis Message ID: ${messageId}`);
...
async function run() {
    ...
    if (args.length < 5) {
        console.error("Usage: node 7_update_member.js <SmartWallet> <OwnerKey> <OrgDIDIndex> <UserDID> <Role> [nonce]");
        process.exit(1);
    }
