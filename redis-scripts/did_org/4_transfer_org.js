...
function encodeTransferOrg(orgDIDIndex, orgDID, newOwner) {
    const abiCoder = AbiCoder.defaultAbiCoder();
    const orgDIDHash = keccak256(solidityPacked(["string"], [orgDID]));
    const encodedData = abiCoder.encode(["bytes32", "address"], [orgDIDHash, newOwner]);
...
async function publishEvent(targetWalletAddress, ownerPrivateKey, orgDIDIndex, orgDID, newOwner, nonce = "0") {
    const provider = new JsonRpcProvider(RPC_URL);
    const ownerWallet = new Wallet(ownerPrivateKey);
    const currentBlockNum = await provider.getBlockNumber();
    const currentBlock = (currentBlockNum + 100).toString(); 

    const { callData, innerData } = encodeTransferOrg(orgDIDIndex, orgDID, newOwner);
    ...
    const event = {
        id: `evt_transfer_org_${Date.now()}`,
        type: "TRANSFER_ORG_OWNER",
        payload: {
    ...
    try {
        ...
        console.log(`✅ Success! TRANSFER_ORG_OWNER Redis Message ID: ${messageId}`);
...
async function run() {
    ...
    if (args.length < 5) {
        console.error("Usage: node 4_transfer_org.js <SmartWallet> <OwnerKey> <OrgDIDIndex> <OrgDID> <NewOwner> [nonce]");
        process.exit(1);
    }
