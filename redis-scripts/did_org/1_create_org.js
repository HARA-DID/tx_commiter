...
async function publishEvent(targetWalletAddress, ownerPrivateKey, orgDID, nonce = "0") {
    ...
    const encodedData = encodeCreateOrg(orgDID);
    ...
    const abiCoder = AbiCoder.defaultAbiCoder();
    const innerData = abiCoder.encode(["string"], [orgDID]);

    const event = {
        id: `evt_create_org_${Date.now()}`,
        type: "CREATE_ORG",
        payload: {
...
async function run() {
    ...
    if (args.length < 3) {
        console.error("Usage: node 1_create_org.js <SmartWallet> <OwnerPrivateKey> <OrgDID> [nonce]");
        process.exit(1);
    }
    await publishEvent(args[0], args[1], args[2], args[3] || "0");
    redis.quit();
}
