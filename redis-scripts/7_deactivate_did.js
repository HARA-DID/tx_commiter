...
function encodeLifecycleDID(didIndex, txType) {
    const abiCoder = AbiCoder.defaultAbiCoder();
    const encodedArgs = abiCoder.encode(["uint256"], [didIndex]);
...
async function publishEvent(targetWalletAddress, ownerPrivateKey, didIndex, txType, eventType) {
    const provider = new JsonRpcProvider(RPC_URL);
    const ownerWallet = new Wallet(ownerPrivateKey);
    const currentBlockNum = await provider.getBlockNumber();
    const currentBlock = (currentBlockNum + 100).toString(); 

    const encodedData = encodeLifecycleDID(didIndex, txType);
...
async function run() {
    ...
    if (args.length < 4) {
        console.error("Usage: node publish_deactivate_did.js <SmartWallet> <OwnerKey> <didIndex> <DEACTIVATE|REACTIVATE>");
        process.exit(1);
    }
    const txType = args[3] === "REACTIVATE" ? 4 : 3;
    await publishEvent(args[0], args[1], args[2], txType, args[3]);
    redis.quit();
}
