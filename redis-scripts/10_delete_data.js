...
const { Wallet, keccak256, solidityPacked, solidityPackedKeccak256, getBytes, JsonRpcProvider, AbiCoder } = require('ethers');
...
function encodeDeleteData(didIndex, key) {
    const abiCoder = AbiCoder.defaultAbiCoder();
    const encodedArgs = abiCoder.encode(["uint256", "string"], [didIndex, key]);
    const txType = 7;
...
async function publishEvent(targetWalletAddress, ownerPrivateKey, didIndex, key) {
    const provider = new JsonRpcProvider(RPC_URL);
    const ownerWallet = new Wallet(ownerPrivateKey);
    const currentBlockNum = await provider.getBlockNumber();
    const currentBlock = (currentBlockNum + 100).toString(); 

    const encodedData = encodeDeleteData(didIndex, key);
    ...
    try {
        ...
        console.log(`✅ Success! DeleteData Redis Message ID: ${messageId}`);
    } catch (error) {
...
async function run() {
    ...
    if (args.length < 4) {
        console.error("Usage: node publish_delete_data.js <SmartWallet> <OwnerKey> <didIndex> <KeyName>");
        process.exit(1);
    }
