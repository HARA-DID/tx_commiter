...
function encodeStoreData(didIndex, propertyKey, data) {
    const abiCoder = AbiCoder.defaultAbiCoder();
    const encodedArgs = abiCoder.encode(
        ["uint256", "string", "string"],
        [didIndex, propertyKey, data]
    );
    const txType = 6;
...
async function publishEvent(targetWalletAddress, ownerPrivateKey, didIndex, propertyKey, data) {
    ...
    const encodedData = encodeStoreData(didIndex, propertyKey, data);
...
async function run() {
    ...
    if (args.length < 5) {
        console.error("Usage: node publish_store_data.js <0xSmartWallet> <0xOwnerPrivateKey> <didIndex> <propertyKey> <data>");
        process.exit(1);
    }
    await publishEvent(args[0], args[1], args[2], args[3], args[4]);
    redis.quit();
}
