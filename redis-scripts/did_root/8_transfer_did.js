...
function encodeTransferDID(didIndex, newOwner) {
    const abiCoder = AbiCoder.defaultAbiCoder();
    const encodedArgs = abiCoder.encode(["uint256", "address"], [didIndex, newOwner]);
    const txType = 5;
...
async function publishEvent(targetWalletAddress, ownerPrivateKey, didIndex, newOwner) {
    const provider = new JsonRpcProvider(RPC_URL);
    const ownerWallet = new Wallet(ownerPrivateKey);
    const currentBlockNum = await provider.getBlockNumber();
    const currentBlock = (currentBlockNum + 100).toString(); 

    const encodedData = encodeTransferDID(didIndex, newOwner);
    ...
    try {
        ...
        console.log(`✅ Success! TransferDID Redis Message ID: ${messageId}`);
    } catch (error) {
...
async function run() {
    ...
    if (args.length < 4) {
        console.error("Usage: node publish_transfer_did.js <SmartWallet> <OwnerKey> <didIndex> <NewOwner>");
        process.exit(1);
    }
